...
|---------|---------|-------------|------------|--------------|
| **[LLM-Augmented Workflow](https://docs.aws.amazon.com/wellarchitected/latest/generative-ai-lens/agentic-ai.html#llm-augmented-workflows)** | - | Code paths are largely deterministic, with certain activities augmented with LLMs to make decisions (e.g., document classification and routing) | ⭐ Low | Minimal |
...
| **[Prompt Chaining](https://www.anthropic.com/engineering/building-effective-agents)** | Sequential Prompting | Decomposes tasks into sequential activities where each LLM call processes previous output | ⭐⭐ Low-Medium | Low |
...
| **Single-Agent (ReAct)** | • Customer support queries with data lookup<br>• Research assistance gathering information from multiple sources<br>• Multi-activity tasks requiring external data access<br>• Computer use implementations (UI automation, file operations) |
...
| **Autonomous Agent (ReAct Loop)** | • **[SWE-bench task resolution](https://www.anthropic.com/engineering/building-effective-agents)**: Understand codebase → locate bugs → implement fixes → test<br>• Open-ended problem solving with unpredictable activities<br>• Computer use tasks (UI testing, system administration) |
...
| **Prompt Chaining** | ✅ Task has clear, fixed subtask boundaries<br>✅ Each activity naturally builds on previous output<br>✅ [Programmatic validation gates](https://www.anthropic.com/engineering/building-effective-agents) needed between activities<br>✅ Debugging individual activities is important |
...
| **Review & Critique** | ✅ Quality assurance is critical<br>✅ [Dedicated verification activity](https://learn.microsoft.com/en-us/azure/architecture/ai-ml/guide/ai-agent-design-patterns) improves outcomes<br>✅ Clear evaluation criteria exist<br>✅ Polished, validated results worth additional cost |
...
| **Autonomous Agent (ReAct Loop)** | ✅ Problem requires [environmental feedback](https://www.anthropic.com/engineering/building-effective-agents) at each activity<br>✅ Activities are unpredictable or open-ended<br>✅ Task benefits from real-time adaptation<br>✅ Sophisticated problem-solving capability needed |
...
| **Prompt Chaining** | ❌ [Tasks require flexibility or adaptation](https://www.anthropic.com/engineering/building-effective-agents)<br>❌ Cannot define fixed structure upfront<br>❌ Activities might need to be skipped or reordered |
...
| **Autonomous Agent (ReAct Loop)** | ❌ Task activities are predictable and structured<br>❌ [Requires extensive oversight](https://www.anthropic.com/engineering/building-effective-agents) but resources unavailable<br>❌ [Compounding errors](https://www.anthropic.com/engineering/building-effective-agents) too risky<br>❌ Cost control critical |
...
| **Prompt Chaining** | • Define clear subtask boundaries<br>• [Validation gates between activities](https://www.anthropic.com/engineering/building-effective-agents)<br>• Balance latency vs accuracy | Activity-by-activity validation | ⚙️⚙️ Medium |
...
| **Prompt Chaining** | → **Sequential Multi-Agent** when activities need specialization<br>→ **Coordinator** when tasks become dynamic |
...
5. **[Environmental Feedback](https://www.anthropic.com/engineering/building-effective-agents)**: Autonomous agents require 'ground truth' from environment at each activity for effectiveness.

6. **[Transparency](https://www.anthropic.com/engineering/building-effective-agents)**: Explicitly show planning activities for debugging and trust.
